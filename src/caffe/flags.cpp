...
bool ParseCaffeCommandLineFlags(int* pargc, char*** pargv) {
  ...
  for (int i = 1; i < *pargc; ++i) {
    ...
    if (arg == "--help") {
      ...
      for (const auto& help_msg : CaffeFlagsRegistry()->HelpMessage()) {
    ...
    if (arg[0] != '-' || arg[1] != '-') {
      GlobalInitStream()
          << "Caffe flag: commandline argument does not match --name=var "
             "or --name format: "
    ...
    if (!CaffeFlagsRegistry()->Has(key)) {
      GlobalInitStream() << "Caffe flag: unrecognized commandline argument: "
                         << arg << std::endl;
      success = false;
      break;
    }
...
template <>
bool CaffeFlagParser::Parse<int>(const string& content, int* value) {
  try {
...
template <>
bool CaffeFlagParser::Parse<int64_t>(const string& content, int64_t* value) {
  try {
...
template <>
bool CaffeFlagParser::Parse<double>(const string& content, double* value) {
  try {
...
template <>
bool CaffeFlagParser::Parse<bool>(const string& content, bool* value) {
